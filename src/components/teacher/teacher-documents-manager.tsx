...
const initialForm: FormState = {
  title: "",
  document_type: "support_material",
  notes: "",
  usable_by_ai: true,
};
...
export function TeacherDocumentsManager({
  ...
  async function reloadDocuments() {
    ...
    if (error) {
      showError(`Erro ao recarregar materiais: ${error.message}`);
      return;
    }
  ...
  function validateForm() {
    if (!form.title.trim()) {
      showError("Informe o título do material.");
      return false;
    }
  ...
  async function handleUpload() {
    ...
    const filePath = `teacher/${currentUserId}/${form.document_type}/${timestamp}_${safeName}`;
    ...
    if (insert.error) {
      ...
      showError(`Erro ao registrar material: ${insert.error.message}`);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Novo material privado</h2>
      ...
      <div className="rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Meus materiais</h2>

        {sortedDocuments.length === 0 ? (
          <p className="mt-4 text-sm text-slate-500">Nenhum material enviado ainda.</p>
        ) : (
